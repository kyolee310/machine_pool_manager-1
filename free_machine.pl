...
sub print_output{
	my $str = shift @_;
	my $ts = print_time();
	my $outstr = "$ts [FREE_MACHINE] [LOG]\t" . $str;
	print $outstr . "\n";
...
sub print_error{
	my $str = shift @_;
	my $ts = print_time();
	my $outstr = "$ts [FREE_MACHINE] [ERROR]\t" . $str;
	print $outstr . "\n";
	system("echo \"$outstr\" >> $LOGFILE");
	exit(1);
};
...
if( @ARGV < 1 ){
	print_error("USAGE : ./free_machine.pl <MACHINE IPs>");
};
...
print_output("FREEING MACHINE IPs: { $input_str }");

validate_input($input_str);

free_machine($input_str);

print_output("FREED MACHINE IPs: { $input_str }");
...
sub validate_input{
	...
	foreach my $ip (@ip_lst ){
		if( !($ip =~ /\d+\.\d+\.\d+\.\d+/) ){
			...
			print_error("MACHINE $ip DOES NOT EXIST IN RECORD !!");
		};
	};
