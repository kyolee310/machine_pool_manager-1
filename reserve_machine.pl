...
sub print_output{
	my $str = shift @_;
	my $ts = print_time();
	my $outstr = "$ts [RESERVE_MACHINE] [LOG]\t" . $str;
	print $outstr . "\n";
...
sub print_error{
	my $str = shift @_;
	my $ts = print_time();
	my $outstr = "$ts [RESERVE_MACHINE] [ERROR]\t" . $str;
	print $outstr . "\n";
	system("echo \"$outstr\" >> $LOGFILE");
	print_log_space();
	exit(1);
...
if( @ARGV < 2 ){
	print_error("USAGE : ./reserve_machine.pl <OWNER> <MACHINE IPs>");
};
...
print_output("RESERVING MACHINE: { $input_str }");

validate_input($input_str);

reserve_machine($owner, $input_str);

print_output("RESERVED MACHINE: { $input_str }");
...
sub validate_input{
	...
	foreach my $ip (@ip_lst ){
		if( !($ip =~ /\d+\.\d+\.\d+\.\d+/) ){
			print_error("INVALID INPUT $ip !!");
		}elsif( is_ip_already_reserved($ip) ){
			print_error("MACHINE $ip ALREADY RESERVED !!");
		};
	};
